...
describes what users get. Pick a format with `-o`, or let ankhang choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
ankhang <command> -o table   # aligned columns for reading
ankhang <command> -o jsonl   # one JSON object per line, for piping
ankhang <command> -o json    # a single JSON array
ankhang <command> -o csv     # spreadsheet friendly
ankhang <command> -o tsv     # tab-separated
ankhang <command> -o url     # just the URL column
ankhang <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
ankhang <command> --fields id,title,url
```
...
```bash
ankhang <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
ankhang <command>            # a table, because this is a terminal
ankhang <command> | wc -l    # JSONL, because this is a pipe
```
